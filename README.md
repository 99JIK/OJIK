...
채점 언어를 4종에서 8종으로 늘리고, 푼 사람끼리 풀이를 나누는 자리를 만들었습니다.

테스트케이스가 제출자에게 새던 결함도 이 버전에서 고쳤습니다. 아래 고친 것을 보세요.

**강의**

- 전역 역할에 `instructor` 추가. 서열은 사용자 < 강사 < 출제자 < 관리자
- 강의(컬렉션) 개설은 강사부터. 운영은 컬렉션 단위로 본다: 만든 사람, 조교(manager), 출제자 이상
- 강의 전용 문제. 강사는 자기 강의에 묶인 문제만 내고 고침. 공개 아카이브 목록에는 안 나오고,
  소속을 바꾸는 건 출제자 이상만. 강의를 지우면 딸린 문제도 같이 지워짐
- 수강생 명단 화면. 진도(강의 안 문제 중 맞힌 수), 조교 지정, 한 명씩 빼기
- 만드는 화면을 다시 짬. 종류를 카드로 고르고 각각 무엇에 쓰는지 한 줄로 설명.
  고르면 정해지는 축 값은 접어 두되 한국어로 보여 줌. 주소는 제목에서 자동 생성.
  만들면 바로 내용을 채우는 화면으로 이동
- 목록에 상태(진행 중, 예정, 종료, 상시), 문제 수, 인원 표시. 비어 있는 것이 눈에 띔

**작성**

- 문제 본문, 입력, 출력, 힌트와 교재 본문을 마크다운 편집기로 씀.
  쓰기 / 나란히 / 미리보기 셋 중에 고르고, 나란히가 기본. 타자가 멈추면 미리보기가 따라옴
- SVG 그림판. 선, 화살표, 네모, 타원, 자유선, 글자. 격자에 맞춰 그리고 결과 SVG 를
  본문에 그대로 넣음. 파일이 안 생기고 도형 열 개짜리가 수백 바이트.
  선 색 기본값이 본문 글자색이라 다크 모드에서도 보임. 넣은 그림은 다시 열어 고칠 수 있음
- 수식(KaTeX), 도식과 그래프(mermaid)는 전부터 렌더는 됐고, 이제 쓰면서 확인할 수 있음
- 그림 올리기. 붙여넣거나 끌어다 놓으면 올라가고 본문에 마크다운으로 끼워짐.
  png, jpeg, gif, webp 만 받고 한 장 4MB. 내용 앞바이트로 형식을 확인해 이름만 바꾼 파일을 막음
- SVG 는 안 받음. 임의의 SVG 는 script 를 품을 수 있어서, 나중에 파일을 그대로 열게 되면
  그 순간 XSS 경로가 됨. 도식은 mermaid 로 본문에 직접 씀
- 문제 등록 폼에서 소속을 고름. 강의 화면에서 바로 새 문제를 만들 수 있고,
  문제 고르기에 그 강의 전용 문제가 같이 나옴

**사용자 프로필**

- 맞힌 문제, 언어별 제출, 판정 분포, 최근 12주 활동, 순위
- 로그인했으면 보는 사람과 비교. 나만 푼 문제, 둘 다 푼 문제, 상대만 푼 문제와 그 목록
- 판정을 아직 감추는 컬렉션의 제출은 집계에서 빠짐
...
- 자동 테스트 84건 (판정 11, 큐 10, 순위표 10, 마크다운 19, enum 대조 7, 풀이 규칙 10, 그림판 17)
- `e2e:teaching` 추가 43건. 권한은 라우트마다 따로 붙는 것이라 라우트 단위로 훑음. 그림 올리기 포함
...
- 컬렉션별 `manager` 가 이름뿐이던 문제. 접근 헬퍼는 인정하는데 쓰기 라우트가 전부 전역
  staff 로 막혀 있어서, 컬렉션 운영자가 자기 컬렉션을 못 고쳤음
- 만든 사람이 비공개 컬렉션을 자기 목록에서 못 보던 문제. 목록 조건이 멤버만 봤음
- 목록의 문제 수가 늘 0 이던 문제. 상관 서브쿼리에서 바깥 컬럼을 drizzle 보간으로 쓰면,
  조인이 없는 쿼리에서는 `"id"` 만 나가서 서브쿼리 안의 같은 이름에 붙었음. 바깥 테이블
  이름을 직접 적게 고침
- 컬렉션 만드는 화면이 축 이름을 영어로 그대로 보여 주던 문제 (참가 members, public)
- 살균기의 SVG 허용이 동작한 적이 없던 문제. `ALLOWED_URI_REGEXP` 는 URI 속성만 보는 게
  아니라 URI_SAFE 로 표시 안 된 모든 값을 검사해서, `fill="none"` 이나 `viewBox` 가 통째로
  떨어져 나갔음. 그림이 속성 없는 껍데기로만 남았음
- 프론트가 권한을 문자열 비교로 판정하던 문제. core 의 `atLeast` 를 쓰게 고침
- 워커 중복 기동 검사가 기동 때만 돌던 문제. 등록 행이 사라지면 여럿이 동시에 떠서
  서로의 러너 컨테이너를 지웠음. 하트비트마다 등록이 아직 자기 것인지 보게 함
...
```bash
npm run db:migrate       # judge_workers.pid, language enum 값 4개, solutions 두 테이블,
                         # role enum 의 instructor, problems.owner_collection_id
